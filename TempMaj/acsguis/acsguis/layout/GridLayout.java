...
public class GridLayout implements PanelLayout<ComponentStyleManager>
{
    ...
    public int getX(ComponentStyleManager target) {
        ...
        return direction == GridDirection.HORIZONTAL ? (getWidth(target.getParent().getWidth())+spacing)*(cache.get(target)%elementsPerLine) : (getWidth(target.getParent().getWidth())+spacing)*(cache.get(target)/elementsPerLine);
    }
    ...
    public int getY(ComponentStyleManager target) {
        ...
        return direction == GridDirection.VERTICAL ? (getHeight(target.getParent().getHeight())+spacing)*(cache.get(target)%elementsPerLine) : (getHeight(target.getParent().getHeight())+spacing)*(cache.get(target)/elementsPerLine);
    }

    @Override
    public int getWidth(ComponentStyleManager target) {
        return getWidth(target.getParent().getWidth());
    }

    @Override
    public int getHeight(ComponentStyleManager target) {
        return getHeight(target.getParent().getHeight());
    }
