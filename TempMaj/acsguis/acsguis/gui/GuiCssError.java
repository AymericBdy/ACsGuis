...
public class GuiCssError extends GuiFrame
{
    public GuiCssError(String guiName, Throwable e)
    {
        ...
        error.addResizeListener((w, h) -> {error.getStyle().setWidth(w-20); error.getStyle().setHeight(h-20);});
        ...
        getStyle().setWidth(r.getScaledWidth()-20);
        getStyle().setHeight(r.getScaledHeight()-20);
        addResizeListener((w, h) -> {getStyle().setWidth(w-20); getStyle().setHeight(h-20);});
    }
    ...
    public GuiCssError()
    {
        ...
        summary = new GuiScrollPane(0, 0, r.getScaledWidth()-20, r.getScaledHeight()-20);

        summary.add(new GuiLabel(0, 0, summary.getWidth(), 20, "Click on any css sheet to view it, then press escape to go back").getStyle().setPaddingLeft(2).setPaddingTop(2).getOwner());
        int i = 1;
        for (Map.Entry<ResourceLocation, Map<CompoundCssSelector, Map<EnumCssStyleProperties, CssStyleProperty<?>>>> entry : DnxCssParser.getCssStyleSheets().entrySet()) {
            ResourceLocation res = entry.getKey();
            Map<CompoundCssSelector, Map<EnumCssStyleProperties, CssStyleProperty<?>>> m = entry.getValue();
            summary.add(new GuiLabel(0, i*22, summary.getWidth(), 20, "+ Style sheet : " + res).getStyle().setPaddingLeft(2).setPaddingTop(2).getOwner().addClickListener((x, y, b) -> {
                remove(summary);
                ...
                error.addResizeListener((w, h) -> {
                    error.getStyle().setWidth(w - 20);
                    error.getStyle().setHeight(h - 20);
                });
