...
import fr.aym.acsguis.component.GuiButton;
...
public class GuiComboBox extends GuiPanel {
	...
	public boolean handleProperty(EnumCssStyleProperties property, EnumSelectorContext context, ComponentStyleManager target) {
		...
		{
			int comboBoxHeight;
			if (guiComboBoxButton.getStyle().getVerticalSize() == GuiConstants.ENUM_SIZE.RELATIVE) {
				comboBoxHeight = (int) (guiComboBoxButton.getParent().getHeight() * guiComboBoxButton.getStyle().getRelativeHeight());
			} else {
	...
	public void developComboBox()
	{
		if(!isDeveloped()) {
			...
			guiComboBoxButton.getStyle().setHeight(comboBoxHeight);
			guiComboBoxButton.getStyle().setVerticalSize(GuiConstants.ENUM_SIZE.ABSOLUTE);
			
			for (GuiEntryButton entryButton : entriesButton) {
				entryButton.getStyle().setY(2 + comboBoxHeight + 12 * entryButton.getEntryId());
	...
	public void retractComboBox()
	{
		if(isDeveloped()) {
			...
			guiComboBoxButton.getStyle().setRelativeHeight(1);
	...
	protected void updateEntriesButtons()
	{
		...
		for(String entry : entries) {
			...
			entryButton.getStyle().setRelativeWidth(1);
