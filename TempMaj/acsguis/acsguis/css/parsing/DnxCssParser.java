...
public class DnxCssParser
{
     ...
     */
    public static final ResourceLocation DEFAULT_STYLE_SHEET = new ResourceLocation(CssGuiManager.RES_LOC, "css/default_style.css");
    /**
    ...
    public static void parseCssSheet(ResourceLocation location) {
        ...
        ICSSVisitor visitor = new DnxCssVisitor(location, cssStyleSheets.get(location));
        CSSVisitor.visitCSS(aCSS, visitor);
        CssGuiManager.log.info("[CSS] Loaded css style sheet "+location);
    ...
    public static CssStackElement getStyleFor(ComponentStyleManager component) {
        ...
        if(parent instanceof GuiFrame) {
            ...
            if(parent.getCssId() == null || !parent.getCssId().equals("css_debug_pane"))
                CssGuiManager.log.warn("Parent gui frame of " + component.getOwner() + " was not found, cannot apply its style !");
            cssSheets.add(DEFAULT_STYLE_SHEET);
        }
        ...
        for(ResourceLocation sheet : cssSheets)
        {
            if(!cssStyleSheets.containsKey(sheet))
                CssGuiManager.log.warn("Style sheet "+sheet+" not loaded !");
            else
